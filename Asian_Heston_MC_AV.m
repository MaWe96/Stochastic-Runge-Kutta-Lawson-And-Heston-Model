...
for i = 1:Nt -1
    ...
    X1 {2}(i+1 ,:) = X1 {2}(i ,:) + kappa *( theta - X1 {2}(i ,:))*h + 47 sigma*sqrt(X1 {2}(i ,:)).* dW1 {2}(i ,:);
    % ensure volatility is non-negative
    X1 {2}(i+1 ,:) = max(X1 {2}(i+1 ,:), 0);
    % generate asset price paths for antithetics variate
    X2 {1}(i+1 ,:) = X2 {1}(i ,:) .* exp ((r - 0.5* X2 {2}(i ,:))*h + sqrt(X2 {2}(i ,:)).* dW2 {1}(i ,:));
    % generate asset volatility paths for antithetics variate
    X2 {2}(i+1 ,:) = X2 {2}(i ,:) + kappa *( theta - X2 {2}(i ,:))*h +sigma*sqrt(X2 {2}(i ,:)).* dW2 {2}(i ,:);
    % ensure volatility is non-negative
